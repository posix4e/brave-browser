...
Config.prototype.update = function (options) {
  ...
  } else if (options.target_arch === 'x86') {
  ...
  if (options.android) {
    this.srcDir = path.join(this.rootDir, 'browser-android', getNPMConfig(['projects', 'browser-android', 'dir']))
  }

  if (options.gclient_file && options.gclient_file !== 'default') {
    this.gClientFile = options.gclient_file
  } else {
    this.gClientFile = path.join(this.srcDir, '..', '.gclient')
  }
  ...
  this.projectNames.forEach((projectName) => {
    if (!options.android) {
      delete this.projects['browser-android']
      this.projectNames = this.projectNames.filter((value, index, arr) => {
        return value != 'browser-android'
      })
    } else {
      this.projectNames = ['browser-android']
      this.projects = {
        'browser-android': this.projects['browser-android']
      }
    }
  })
}
