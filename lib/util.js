...
const runGClient = (args, options = {}) => {
  ...
  options.cwd = path.dirname(config.gClientFile)
  args = args.concat('--gclientfile', path.basename(config.gClientFile))
...
const util = {
  ...
  buildGClientConfig: () => {
    ...
    if (!fs.existsSync(path.dirname(config.gClientFile))) {
      fs.mkdirSync(path.dirname(config.gClientFile), {recursive: true})
    }
    fs.writeFileSync(config.gClientFile, out)
  },
  ...
  gclientSync: (reset = false, args = [], options = {}) => {
    args = ['sync', '--with_branch_heads', '--with_tags', '-v'].concat(args)
    if (reset) {
      args.push('--reset')
      args.push('--force')
      args.push('--upstream')
